...
max_episodes  = 1000
max_steps   = 1000  # Pendulum needs 150 steps per episode to learn well, cannot handle 20
frame_idx   = 0
batch_size  = 8
explore_steps = 1000  # for random action sampling in the beginning of training
update_itr = 1
AUTO_ENTROPY=True
DETERMINISTIC=False
hidden_dim = 256
...
writer = SummaryWriter('runs/{}_SACv2_{}'.format(datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S"), ENV))
...
if __name__ == '__main__':
    if args.train:
        ...
        for eps in range(max_episodes):
            ...
            writer.add_scalar('reward/train', episode_reward, eps)
            print("Episode: {}, total numsteps: {}, episode steps: {}, reward: {}".format(eps, total_numsteps, episode_steps, round(episode_reward, 2)))
